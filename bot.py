...
# Identidades de navegadores para evitar bloqueios em todas as redes
USER_AGENTS = [
...
def menu_principal(message):
    ...
    texto = (f"👋 **Bot de Downloads**\n\n📊 Plano: **{plano}**\n💡 Saldo: **{restantes}** downloads gratuitos hoje.\n\n"
             "Assine para baixar sem limites de todas as redes:")
...
def validar_pix(call):
    ...
    if res["response"]["status"] == "approved":
        ...
        conn.commit()
        conn.close()
        bot.edit_message_text(f"✅ **Plano {plano_nome} ativado com sucesso!**", call.message.chat.id, call.message.id)
    else:
        bot.answer_callback_query(call.id, "❌ Pagamento ainda não detectado.", show_alert=True)

# --- MOTOR DE DOWNLOAD (UNIVERSAL) ---
...
def baixar(message):
    ...
    sites = ["instagram.com", "tiktok.com", "pin.it", "pinterest.com", "rednote", "xhs"]
    ...
    if any(site in url for site in sites):
        ...
        msg_wait = bot.reply_to(message, "⚡ Analisando link e preparando download...")
        file_name = f"dl_{message.from_user.id}_{random.randint(100,999)}.mp4"
        
        # Proteção extra: Pequena espera para o Pinterest e Instagram
        if "pin.it" in url or "pinterest" in url or "instagram" in url:
        ...
        try:
            ...
            bot.edit_message_text("❌ Erro ao baixar. O link pode estar instável ou protegido. Tente novamente mais tarde.", message.chat.id, msg_wait.message_id)
