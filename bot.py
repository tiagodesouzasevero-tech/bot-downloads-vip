...
# --- DOWNLOADER COM PADRONIZAÇÃO HD 720p/30fps ---
...
def handle_dl(message):
    ...
    # CONFIGURAÇÃO DE QUALIDADE (FOCO DO AJUSTE)
    ydl_opts = {
        ...
        'postprocessor_args': [
            # Lógica FFMPEG: 
            # 1. Se altura > 720, reduz para 720. Senão, mantém original (sem upscale).
            # 2. Fixa 30 FPS.
            # 3. Codec libx264 (máxima compatibilidade).
            '-vf', "scale='if(gt(ih,720),-2,iw)':'if(gt(ih,720),720,ih)',fps=30",
            '-c:v', 'libx264',
            '-crf', '23', # Equilíbrio entre qualidade e peso do arquivo
            '-preset', 'veryfast'
        ],
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Trava de 90s (Mantida conforme regra do RedNote)
        ...
        bot.edit_message_text("❌ Erro ao baixar. Verifique o link.", message.chat.id, msg_p.message_id)

# --- WEBHOOK E SERVIDOR (MANTIDOS 100%) ---
